...
module.exports = async channel => {
    await channel.guild.fetchAuditLogs({ type: "CHANNEL_DELETE" }).then(async audit => {
        ...
        if(whitelist.full.includes(bum.id) || whitelist.kanal.includes(bum.id)) {
            ...
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı #${channel.name} (\`${channel.id}\`) kanalını sildi, güvenli listede olduğu için işlem yapılmadı.`, components:[banla] })
            client.on("interactionCreate", async(int) => {
                if(!client.base.sec.includes(int.member.id)) return
                if(int.customId == "ban") {
                    await channel.guild.members.ban(bum.id, { reason: "Mavera Kanal Silme Koruması" })
            ...
            await channel.guild.members.ban(bum.id, { reason: "Mavera Kanal Silme Koruması" })
            .then(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı #${channel.name} (\`${channel.id}\`) kanalını sildi ve yasaklandı! @everyone` }))
            .catch(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı #${channel.name} (\`${channel.id}\`) kanalını sildi ama __yasaklanamadı__! @everyone` }))
            await ytkapa()
            await channel.clone({ reason: "Mavera Kanal Silme Koruması" }).then(async x => {
                if(channel.parentId !== null) await x.setParent(channel.parentId)
                await x.setPosition(channel.position)
                if(channel.type == "GUILD_CATEGORY") await channel.guild.channel.cache.filter(y => y.parentId == channel.id).forEach(z => z.setParent(x.id))
            })
        }
    })
}
