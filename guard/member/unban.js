...
module.exports = async(uye) => {
    await uye.guild.fetchAuditLogs({ type: "MEMBER_BAN_REMOVE" }).then(async audit => {
        const yes = uye.guild.emojis.cache.find(x => x.name == "mavera_yes")
        const no = uye.guild.emojis.cache.find(x => x.name == "mavera_no")
        ...
        if(whitelist.full.includes(bum.id)) {
            ...
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı ${uye.user} (\`${uye.user.tag}\` - \`${uye.user.id}\`) üyesinin yasağını açtı, güvenli listede olduğu için işlem yapılmadı.`, components:[banla] })
            client.on("interactionCreate", async(int) => {
                if(!client.base.sec.includes(int.member.id)) return
                if(int.customId == "ban") {
                    await uye.guild.members.ban(bum.id, { reason: "Mavera Sağ Tık Unban Koruması" })
            ...
            await uye.guild.members.ban(bum.id, { reason: "Mavera Sağ Tık Unban Koruması" })
            .then(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı ${uye.user} (\`${uye.user.tag}\` - \`${uye.user.id}\`) üyesinin yasağını açtı ve yasaklandı! @everyone` }))
            .catch(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı ${uye.user} (\`${uye.user.tag}\` - \`${uye.user.id}\`) üyesinin yasağını açtı ama __yasaklanamadı__! @everyone` }))
            await ytkapa()
            await uye.guild.members.ban(uye.user.id, { reason: "Yasağı izinsizce açıldığı için tekrar yasaklandı." })
        }
    })
}
