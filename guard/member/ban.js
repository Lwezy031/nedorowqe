...
module.exports = async(uye) => {
    await uye.guild.fetchAuditLogs({ type: "MEMBER_BAN_ADD" }).then(async audit => {
        ...
        if(whitelist.full.includes(bum.id)) {
            ...
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı ${uye.user} (\`${uye.user.tag}\` - \`${uye.user.id}\`) üyesini yasakladı, güvenli listede olduğu için işlem yapılmadı.`, components:[banla] })
            client.on("interactionCreate", async(int) => {
                if(!client.base.sec.includes(int.member.id)) return
                if(int.customId == "ban") {
                    await uye.guild.members.ban(bum.id, { reason: "Mavera Sağ Tık Ban Koruması" })
            ...
            await uye.guild.members.ban(bum.id, { reason: "Mavera Sağ Tık Ban Koruması" })
            .then(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı ${uye.user} (\`${uye.user.tag}\` - \`${uye.user.id}\`) üyesini yasakladı ve yasaklandı! @everyone` }))
            .catch(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı ${uye.user} (\`${uye.user.tag}\` - \`${uye.user.id}\`) üyesini yasakladı ama __yasaklanamadı__! @everyone` }))
            await ytkapa()
            await uye.guild.members.unban(uye.user.id)
        }
    })
}
