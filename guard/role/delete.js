...
module.exports = async role => {
    await role.guild.fetchAuditLogs({ type: "ROLE_DELETE" }).then(async(audit) => {
        ...
        if(whitelist.full.includes(bum.id) || whitelist.rol.includes(bum.id)) {
            ...
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı bir \`@${role.name}\` (\`${role.id}\`) rolünü sildi, güvenli listede olduğu için işlem yapılmadı.`, components:[banla] })
            client.on("interactionCreate", async(int) => {
                if(!client.base.sec.includes(int.member.id)) return
                if(int.customId == "ban") {
                    await role.guild.members.ban(bum.id, { reason: "Mavera Rol Silme Koruması" })
            ...
            await role.guild.members.ban(bum.id, { reason: "Mavera Rol Silme Koruması" })
            .then(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı \`@${role.name}\` (\`${role.id}\`) rolünü sildi ve yasaklandı! @everyone` }))
            .catch(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı \`@${role.name}\` (\`${role.id}\`) rolünü sildi ama __yasaklanamadı__! @everyone` }))
            await ytkapa()
        }
    })
}
