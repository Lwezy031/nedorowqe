...
module.exports = async role => {
    await role.guild.fetchAuditLogs({ type: "ROLE_CREATE" }).then(async(audit) => {
        const yes = role.guild.emojis.cache.find(x => x.name == "mavera_yes")
        const no = role.guild.emojis.cache.find(x => x.name == "mavera_no")
        ...
        if(whitelist.full.includes(bum.id) || whitelist.rol.includes(bum.id)) {
            ...
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı bir rol oluşturdu, güvenli listede olduğu için işlem yapılmadı.`, components:[banla] })
            client.on("interactionCreate", async(int) => {
                if(!client.base.sec.includes(int.member.id)) return
                if(int.customId == "ban") {
                    await role.guild.members.ban(bum.id, { reason: "Mavera Rol Oluşturma Koruması" })
            ...
            await role.guild.members.ban(bum.id, { reason: "Mavera Rol Oluşturma Koruması" })
            .then(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı bir rol oluşturdu ve yasaklandı! @everyone` }))
            .catch(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı bir rol oluşturdu ama __yasaklanamadı__! @everyone` }))
            await ytkapa()
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı bir rol oluşturdu ve yasaklandı! @everyone` })
        }
    })
}
