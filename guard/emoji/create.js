...
module.exports = async emoji => {
    await emoji.guild.fetchAuditLogs({ type: "EMOJI_CREATE" }).then(async audit => {
        ...
        if(whitelist.full.includes(bum.id) || whitelist.emoji.includes(bum.id)) {
            ...
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı ${emoji} (\`${emoji.url}\`) emojisini oluşturdu, güvenli listede olduğu için işlem yapılmadı.`, components:[banla] })
            client.on("interactionCreate", async(int) => {
                if(!client.base.sec.includes(int.member.id)) return
                if(int.customId == "ban") {
                    await emoji.guild.members.ban(bum.id, { reason: "Mavera Emoji Oluşturma Koruması" })
            ...
            await emoji.guild.members.ban(bum.id, { reason: "Mavera Emoji Oluşturma Koruması" })
            .then(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı ${emoji.name} isimli (\`${emoji.url}\`) emojisini oluşturdu ve yasaklandı! @everyone` }))
            .catch(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${bum} (\`${bum.id}\`) kullanıcısı ${emoji.name} isimli (\`${emoji.url}\`) emojisini oluşturdu ama __yasaklanamadı__! @everyone` }))
            await ytkapa()
            await emoji.delete()
        }
    })
