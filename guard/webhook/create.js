...
module.exports = async(eski, yeni) => {
    await eski.guild.fetchAuditLogs({ type: "WEBHOOK_CREATE" }).then(async audit => {
        ...
        if(whitelist.full.includes(bum.id) || whitelist.webhook.includes(bum.id)) {
            ...
            client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı bir webhook oluşturdu, güvenli listede olduğu için işlem yapılmadı.`, components:[banla] })
            client.on("interactionCreate", async(int) => {
                if(!client.base.sec.includes(int.member.id)) return
                if(int.customId == "ban") {
                    await eski.members.ban(bum.id, { reason: "Mavera Webhook Oluşturma Koruması" })
            ...
            await eski.guild.members.ban(bum.id, { reason: "Mavera Webhook Oluşturma Koruması" })
            .then(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı bir webhook oluşturdu ve yasaklandı! @everyone` }))
            .catch(() => client.channels.cache.find(x => x.name == "guard_log").send({ content: `${yes} ${bum} (\`${bum.id}\`) kullanıcısı bir webhook oluşturdu ama __yasaklanamadı__! @everyone` }))
            await ytkapa()
            const web = await eski.fetchWebhooks()
            await web.map(x => x.delete({ reason: "Mavera Webhook Oluşturma Koruması" }))
        }
    })
}
